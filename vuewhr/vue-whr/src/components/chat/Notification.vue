      ...
      <el-main style="padding:0px">
        ...
        <el-collapse accordion style="width: 90%" @change="handleChange" v-model="mid">
          <el-collapse-item v-for="(msg,index) in sysmsgs" :key="index" :name="msg.msgContent.id">
            ...
            <div
              style="display: flex;justify-content: flex-start;align-items: center;border-style: solid;border-width: 1px;
              border-color: #409eff;border-radius: 5px;padding: 4px 0px 4px 8px;box-sizing: border-box;height: 100%"
...
export default {
  ...
  methods: {
    handleChange(newVal) {
      ...
      var _this = this;
      this.putRequest("/chat/markread", {
        flag: this.mid
      }).then(resp => {
        if (resp && resp.status == 200) {
          //更新完消息的阅读状态后，再重新获取更新后的所有消息
          _this.initSysMsgs();
        }
      });
    },
