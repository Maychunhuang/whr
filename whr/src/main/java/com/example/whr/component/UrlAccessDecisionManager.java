 ...
 *
 * 3.遍历collection，同时查看当前用户的角色列表中是否具备需要的权限，
 * 如果具备就直接返回，否则就抛异常。
 *
 * 4.这里涉及到一个all和any的问题：假设当前用户具备角色A、角色B，
 * 当前请求需要角色B、角色C，那么是要当前用户要包含所有请求角色才算授权成功还是只要包含一个就算授权成功？
 * 我这里采用了第二种方案，即只要包含一个即可。
 * 小伙伴可根据自己的实际情况调整decide方法中的逻辑。
 * @author huangchunmei
 * @create 2019/8/18 9:37
 */
@Component
