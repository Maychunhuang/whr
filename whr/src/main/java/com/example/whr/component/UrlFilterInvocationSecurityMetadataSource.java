package com.example.whr.component;
 ...
 * 然后将这个请求url和数据库中查询出来的所有url pattern一一对照，
 * 看符合哪一个url pattern，然后就获取到该url pattern所对应的角色，
 * 当然这个角色可能有多个，所以遍历角色，最后利用SecurityConfig.createList方法来创建一个角色集合。
 *
 ...
 *
 * 4.如果getAttributes(Object o)方法返回null的话，意味着当前这个请求不需要任何角色就能访问，
 * 甚至不需要登录。但是在我的整个业务中，并不存在这样的请求，
 * 我这里的要求是，所有未匹配到的路径，都是认证(登录)后可访问，
 * 因此我在这里返回一个ROLE_LOGIN的角色，这种角色在我的角色数据库中并不存在，
 * 因此我将在下一步的角色比对过程中特殊处理这种角色。
 *
 * 5.如果地址是/login_p，这个是登录页，不需要任何角色即可访问，直接返回null。
 *
 * 6.getAttributes(Object o)方法返回的集合最终会来到AccessDecisionManager类中，
 * 接下来我们再来看AccessDecisionManager类。
 * @author huangchunmei
 * @create 2019/8/14 15:29
 */
@Component
